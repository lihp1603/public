...
int64_t TimeUTCMicros() {
	...
	return (static_cast<int64_t>(time.time) * MediaEffectSdk::kNumMicrosecsPerSec +
		static_cast<int64_t>(time.millitm) * MediaEffectSdk::kNumMicrosecsPerMillisec);

#else//linux
	struct timeval time;
    gettimeofday(&time, NULL);
	// Convert from second (1.0) and microsecond (1e-6).
    return (static_cast<int64_t>(time.tv_sec) * PublicTool::kNumMicrosecsPerSec +
		time.tv_usec);
