...
bool CQueryResult::nextResult()
{
    ...
    do
	{
		...
		if (status > 0)                     // error
		{
            ELogPrint("get next result status error = %d, status = %s, info = %s.", m_dbOpt->errorNum(), m_dbOpt->stateInfo(), m_dbOpt->errorInfo());
			return false;
		}
		...
		{
            ELogPrint("get more result error = %d, status = %s, info = %s.", m_dbOpt->errorNum(), m_dbOpt->stateInfo(), m_dbOpt->errorInfo());
			return false;
		}
...
int CPreparedStmt::prepare(const char* stmtStr, unsigned long length)
{
	...
	if (rc != 0)
	{
        ELogPrint("prepare stmt error = %d, status = %s, info = %s.", errorNum(), stateInfo(), errorInfo());
...
int CPreparedStmt::execute()
{
	...
	{
        ELogPrint("execute stmt error = %d, status = %s, info = %s.", errorNum(), stateInfo(), errorInfo());
		return DoPreStmtCmmError;
	}
	
	if (m_resultMode == 0 && mysql_stmt_store_result(m_stmt) != 0)
	{
        LogPrint("get all stmt execute result error = %d, status = %s, info = %s.", errorNum(), stateInfo(), errorInfo());
	}
...
bool CPreparedStmt::nextResult()
{
	...
	if (rc > 0)
	{
        LogPrint("get next result data error = %d, status = %s, info = %s.", errorNum(), stateInfo(), errorInfo());
	}
...
bool CPreparedStmt::toNextRow()
{
	...
	if (rc != MYSQL_NO_DATA)
	{
        LogPrint("get next row data error = %d, status = %s, info = %s.", errorNum(), stateInfo(), errorInfo());
	}
...
int CDBOpertaion::executeSQL(const char* sql, const unsigned int len)
{
    ...
    {
        COSMutexLocker tmpLocker(&m_mysqlMutex);
        if(mysql_real_query(&m_mysqlHandler, sql, len) != 0)
        {
            if (2013 == errorNum())
            {
                if (mysql_real_query(&m_mysqlHandler, sql, len) != 0)
                {
                    ...
                    ELogPrint("redo execute sql error = %d, status = %s, info = %s, sql len = %d, data = %s",
            ...
            {
                std::string strSql(sql, len);
                ELogPrint("execute sql error = %d, status = %s, info = %s, sql len = %d, data = %s",
                errorNum(), stateInfo(), errorInfo(), len, strSql.c_str());
                return DoSqlCmmError;
            }
        }
...
int CDBOpertaion::queryTableAllResult(const char* sql, const unsigned int len, CQueryResult*& qResult)
{
	...
	if (result == NULL)
	{
		...
		{
            ELogPrint("get all sql result error = %d, status = %s, info = %s.", errorNum(), stateInfo(), errorInfo());
...
int CDBOpertaion::queryTableResult(const char* sql, const unsigned int len, CQueryResult*& qResult)
{
	...
	if (result == NULL)
	{
		if (errorNum() != 0)
		{
            ELogPrint("get single sql result error = %d, status = %s, info = %s.", errorNum(), stateInfo(), errorInfo());
...
int CMySql::createDBOpt(CDBOpertaion*& dbOpt, const char* host, const char* user, const char* passwd, const char* db, unsigned int port)
{
	...
	do
	{
		...
		if(ret != 0)
		{
            ELogPrint("set charset name error = %d, status = %s, info = %s.", ::mysql_errno(handler), ::mysql_sqlstate(handler), ::mysql_error(handler));
			break;
		}
		...
		if(ret != 0)
		{
            ELogPrint("set reconnect error = %d, status = %s, info = %s.", ::mysql_errno(handler), ::mysql_sqlstate(handler), ::mysql_error(handler));
			break;
		}
		...
		{
			ret = -1;
            ELogPrint("connect mysql server error = %d, status = %s, info = %s.", ::mysql_errno(handler), ::mysql_sqlstate(handler), ::mysql_error(handler));
			break;
		}
